...
public class ItunesDAO {
	...
	public List<Track> getCanzoniGrafo(Genre genre){
		final String sql = "SELECT * "
				...
				+ "WHERE t.GenreId = g.GenreId AND g.GenreId = ?";
	...
	public List<Adiacenze> getAllAdiacenze(Genre g){
		final String sql = "SELECT t1.TrackId, t2.TrackId, (t1.Milliseconds - t2.Milliseconds) as delta "
		...
		try {
			...
			while (res.next()) {
				result.add(new Adiacenze(res.getInt("t1.TrackId"), res.getInt("t2.TrackId"), res.getDouble("delta")));
			}
			conn.close();
		} catch (SQLException e) {
			e.printStackTrace();
